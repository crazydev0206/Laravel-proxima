...
include('../common/connect.php');
    if(!isset($_GET['id'])) {
?>
<script> window.location="../all_threads/"; </script>
<?php
}
...
if($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  $r=@mysqli_query($dbc,$q);
    ...
    $q="UPDATE threads SET title='$title', description='$description', category='$category' WHERE id='$id'";
    $r=@mysqli_query($dbc,$q);
    if($r) {
        $error2= '<font style="color:green; font-weight:bold;">Post updated successfully.</font>';
    }
    }
...
$error2='There is an error. Try Later.<br>';
    ...
    $q="SELECT * FROM threads WHERE id='$id' LIMIT 1";
    $r=@mysqli_query($dbc,$q);
              ...
              <div class="box-body">
                <div class="form-group">
                  ...
                  <select name="category" class="form-control" required>
                      <option value=""></option>
                              <?php 
                                $q="SELECT name FROM art_categories";
                                $r=@mysqli_query($dbc,$q);
                                if(@mysqli_num_rows($r)!=0) {
                                ...
                                <?php } } ?>
                                <option value="Everything Else" <?php if($row['category']=='Everything Else') echo 'selected'; ?> >Everything Else</option>
                  </select>
                </div>
