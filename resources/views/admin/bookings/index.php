...
if($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  $q="DELETE FROM bookings WHERE id='$v_id'";
...
include('../common/header.php'); ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Bookings
        <small><?php echo @mysqli_num_rows($r); ?> total bookings</small>
      </h1>
      <ol class="breadcrumb">
                ...
                <tbody>
                  ...
                  if(@mysqli_num_rows($r)!=0) {
                    while($row=@mysqli_fetch_assoc($r)) {
                        ...
                        $q3="SELECT from_provider,for_trip FROM custom_offers WHERE id='$for_offer' LIMIT 1";
                        $r3=@mysqli_query($dbc,$q3);
                        $row3=@mysqli_fetch_assoc($r3);
                        
                        $provider=$row3['from_provider'];
                        $for_trip=$row3['for_trip'];
                        
                        $q3="SELECT * FROM providers WHERE id='$provider' LIMIT 1";
                        $r3=@mysqli_query($dbc,$q3);
                        $row3=@mysqli_fetch_assoc($r3);
                   ?>
                <tr>
                  ...
                  <td><?php echo $row3['manager_name'].' <br>('.$row3['business_name'].'<br>'.$row3['email'].')'; ?></td>
                  <td><?php echo $row['book_date']; ?></td>
                  <td>#<?php echo $for_trip; ?></td>  
                  <td>#<?php echo $for_offer; ?></td>  
                  <td>
                    <form action="" method="post">
                    <input type="hidden" name="video_id" value="<?php echo $row['id']; ?>">
                    <button type="submit" class="btn btn-danger" onclick="return confirm('Do you really want to delete this booking?');">Delete</button>
                  </form>
                  </td>
                </tr>
                <?php } } ?>
